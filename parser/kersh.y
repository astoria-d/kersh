...
#define NOT_SUPPORTED(msg)   \
    yyerror(msg);\
    YYABORT

#define LEGACY_C_NG     "legacy C style function definition not supported.\n"
...
parameter_type_list     :   parameter_list                                                                                          {$$ = $1; POST_REDUCE(indx_parameter_type_list_0); }
                        |   parameter_list ',' DOT3                                                                                 {POST_REDUCE(indx_parameter_type_list_1); }
                        ;

parameter_list      :   parameter_declaration                                                                                       {$$ = $1; POST_REDUCE(indx_parameter_list_0); }
                    |   parameter_list ',' parameter_declaration                                                                    {$$ = append_declarator($1, $3); POST_REDUCE(indx_parameter_list_1); }
                    ;

parameter_declaration   :   declaration_specifiers declarator                                                                       {$$ = declare($1, $2); POST_REDUCE(indx_parameter_declaration_0); }
                        |   declaration_specifiers                                                                                  {$$ = $1; POST_REDUCE(indx_parameter_declaration_1); }
                        |   declaration_specifiers abstract_declarator                                                              {POST_REDUCE(indx_parameter_declaration_2); }
                        ;

identifier_list     :   identifier                                                                                                  {NOT_SUPPORTED(LEGACY_C_NG); POST_REDUCE(indx_identifier_list_0); }
                    |   identifier_list ',' identifier                                                                              {NOT_SUPPORTED(LEGACY_C_NG); POST_REDUCE(indx_identifier_list_1); }
                    ;
...
function_definition     :   declaration_specifiers declarator compound_statement                                                    {$$ = alloc_function($1, $2, $3); POST_REDUCE(indx_function_definition_0); }
                        |   declaration_specifiers declarator declaration_list compound_statement                                   {NOT_SUPPORTED(LEGACY_C_NG); POST_REDUCE(indx_function_definition_1); }
                        ;

declaration_list        :   declaration                                                                                             {NOT_SUPPORTED(LEGACY_C_NG); POST_REDUCE(indx_declaration_list_0); }
                        |   declaration_list declaration                                                                            {NOT_SUPPORTED(LEGACY_C_NG); POST_REDUCE(indx_declaration_list_1); }
                        ;
